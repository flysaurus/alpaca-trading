...
export default function WatchlistWidget() {
  ...
  return (
    <div className="bg-[var(--card-bg)] rounded-xl border border-[#1e232b]">
      ...
      <div className="px-4 py-2 border-b border-[#1e232b] flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="relative" ref={dropdownRef}>
            ...
            {showDropdown && (
              <div className={`absolute right-0 top-full mt-1 w-56 bg-[var(--surface-bg)] border border-[#1e232b] rounded-lg shadow-xl z-50 ${
                  ...
                  {watchlists.map((w) => (
                    <div
                      ...
                      className={`flex items-center justify-between px-3 py-2 cursor-pointer transition ${
                        w.id === activeId ? 'bg-amber-500/10 text-amber-400' : 'text-[var(--text-secondary)] hover:bg-[var(--hover-bg)] hover:text-[var(--text-primary)]'
                      }`}
      ...
      {/* Create watchlist */}
      {showCreate && (
        <div className="px-4 py-3 border-b border-[#1e232b] bg-[var(--surface-bg)]/50">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[var(--app-bg)] border border-[#1e232b] rounded-lg px-3 py-1.5 text-xs text-[var(--text-primary)] placeholder-[var(--text-subtle)] focus:outline-none focus:border-amber-500/50"
              autoFocus
            />
            <button
              onClick={createList}
            ...
            <button
              ...
              className="px-2 py-1.5 text-[var(--text-muted)] hover:text-[var(--text-primary)] text-xs transition"
            >
              Cancel
            </button>
          </div>
          ...
          {displaySymbols.map((sym) => {
            ...
            if (!q) {
              return (
                <div
                  key={sym}
                  className={`flex-shrink-0 bg-[var(--surface-bg)] rounded-lg px-3 py-2 min-w-[90px] border border-[var(--border)]/50 relative transition-opacity ${isRemoving ? 'opacity-0' : 'opacity-100'} ${isSpecial ? 'border-amber-500/30' : ''}`}
                >
                  {isSpecial && (
            ...
            return (
              ...
              >
                ...
                <p className="text-[10px] font-bold text-[var(--text-secondary)] tracking-wider">{q.symbol}</p>
                <p className="text-sm font-bold font-[family-name:var(--font-mono)] text-[var(--text-primary)] tabular-nums leading-none mt-0.5">
                  ${q.price.toFixed(2)}
                </p>
                <div className={`flex items-center gap-1.5 mt-1 ${isUp ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}>
                  ...
                  <span className="text-[9px] font-bold font-[family-name:var(--font-mono)] px-1 py-0.5 rounded bg-[var(--app-bg)]/60">
      ...
      <div className="px-4 py-2 border-t border-[#1e232b]">
        {showAdd && (
          <div className="flex gap-2 items-start">
            ...
            <button
              onClick={() => addSymbol()}
              className="px-3 py-2 bg-[var(--green-soft)] hover:bg-[#15803d] text-[var(--text-primary)] text-xs font-bold rounded-lg transition mt-0"
            >
              Add
            </button>
            <button
