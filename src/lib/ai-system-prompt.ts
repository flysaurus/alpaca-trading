...
// Forward-declare interface for AIContext to avoid circular dependency
interface AIContext {
  portfolio?: {
    totalValue?: number
    buyingPower?: number
...
function buildModeInstructions(
  ...
  switch (mode) {
    case 'health':
      return `
MODE: PORTFOLIO HEALTH DIAGNOSTIC
...
export function buildSystemPrompt(
  ...
  const styleName = STYLE_NAMES[context.investorStyle] || 'Growth'
  const riskName =
    (context.riskTolerance || 'moderate').charAt(0).toUpperCase() +
    (context.riskTolerance || 'moderate').slice(1)
  const styleMandate =
...
PORTFOLIO DATA
${context.portfolioSummary || 'No portfolio data available.'}
