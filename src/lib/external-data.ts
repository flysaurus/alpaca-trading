...
export async function getAnalystData(
  ...
  const { data } = await supabase
    ...
    .eq('symbol', symbol)
    .single()
  const cached = data as Record<string, any> | null
  ...
  try {
    ...
    if (total > 0) {
      const score =
        (strongBuy * 5 + buy * 4 + hold * 3 + sell * 2 + strongSell) / total
    ...
    const analystData: AnalystData = {
      ...
      institutionalOwnership: keyStats?.heldPercentInstitutions?.raw || 0,
    ...
    await supabase.from('stock_analysis_cache').upsert({
      ...
      analyst_expires_at: new Date(Date.now() + 24 * 3600000).toISOString(),
    })
...
export async function getNewsSentiment(
  ...
  if (
    ...
    return {
      overall: cached.news_sentiment as 'positive' | 'neutral' | 'negative',
  ...
  try {
    ...
    if (Array.isArray(results) && Array.isArray(results[0])) {
      results[0].forEach((item: { label: string; score: number }) => {
    ...
    const overall =
      netScore > 0.1 ? 'positive' : netScore < -0.1 ? 'negative' : 'neutral'
    ...
    await supabase.from('stock_analysis_cache').upsert({
      ...
      technicals_expires_at: new Date(Date.now() + 3600000).toISOString(),
    })
